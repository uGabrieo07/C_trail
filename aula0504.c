...
main (int argc, char *argv []) 
{
	...
	ld matrizOriginal [numeroLinhas][numeroColunas];
	ld matrizTransposta [numeroColunas][numeroLinhas];

	for (linha = 0; linha < numeroLinhas; ++linha){
		for (coluna = 0; coluna < numeroColunas; ++coluna){
			matrizOriginal [linha][coluna] = strtold (argv [3 + coluna + linha * numeroColunas], &verificacaoMatriz); 
	...
	ObterMatrizTransposta (numeroLinhas, numeroColunas, matrizOriginal, matrizTransposta);
	ExibirMatriz (numeroColunas, numeroLinhas, matrizTransposta);
